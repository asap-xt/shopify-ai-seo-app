...
const jsonLdDefinitionCreated = new Set();

async function ensureJsonLdDefinition(req, shop) {
  if (jsonLdDefinitionCreated.has(shop)) return;
  ...
  const mutation = `
    mutation {
      metafieldDefinitionCreate(definition: {
        namespace: "custom"
        key: "json_ld"
        name: "Product JSON-LD Schema"
        type: "json"
        ownerType: PRODUCT
        description: "Structured data (schema.org) for product pages - auto-generated by indexAIze"
        pin: true
      }) {
  ...
  try {
    ...
    jsonLdDefinitionCreated.add(shop);
  } catch (e) {
    if (!e.message?.includes('in use') && !e.message?.includes('already exists') && !e.message?.includes('taken')) {
      console.error(`[JSON-LD] Definition creation error:`, e.message);
    }
    jsonLdDefinitionCreated.add(shop);
  }
}
...
async function applySEOForLanguage(req, shop, productId, seo, language, options = {}) {
  ...
  try {
    ...
    if (!dryRun) {
      ...
      if (v.jsonLd) {
        try {
          await ensureJsonLdDefinition(req, shop);
