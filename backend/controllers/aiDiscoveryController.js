...
// Background sitemap regeneration endpoint
router.post('/ai-discovery/regenerate-sitemap', validateRequest(), async (req, res) => {
  try {
    console.log('[AI-DISCOVERY] Background sitemap regeneration requested');
    const shop = req.shopDomain;
    ...
    const { generateSitemap } = await import('./sitemapController.js');
    
    // Generate sitemap in background
    console.log('[AI-DISCOVERY] Starting background sitemap generation for shop:', shop);
    ...
    const mockRes = {
      status: (code) => ({ json: (data) => console.log(`[AI-DISCOVERY] Sitemap generation response (${code}):`, data) }),
      json: (data) => console.log('[AI-DISCOVERY] Sitemap generation response:', data)
    };
    
    // Don't await - let it run in background
    generateSitemap(mockReq, mockRes)
      .then(() => {
        console.log('[AI-DISCOVERY] Background sitemap generation completed');
      })
      .catch((error) => {
        console.error('[AI-DISCOVERY] Background sitemap generation failed:', error);
      });
    ...
    console.error('[AI-DISCOVERY] Error starting sitemap regeneration:', error);
    res.status(500).json({ error: error.message });
  }
});
