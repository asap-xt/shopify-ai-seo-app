...
// OPTIMIZED: Results are cached per shop to avoid repeated HTTP requests
  ...
  const defaultSettings = {
    ...
    audience: false,
    reviews: false,
    ratings: false, // DISABLED: app-generated ratings are AI-fabricated. Real ratings come from Judge.me.
...
async function generateLangSchemas(product, seoData, shop, language, shopName = null, primaryDomainUrl = null) {
  ...
  // Check actual availability from variants
  const hasAvailableVariant = product.variants?.edges?.some(v => v.node.availableForSale) || false;
  const availability = hasAvailableVariant 
    ? "https://schema.org/InStock" 
    : "https://schema.org/OutOfStock";
  
  // Get first available variant for SKU/GTIN
  ...
  // AggregateRating is intentionally NOT generated.
  // generateRatingSchemas() fabricates ratings via AI (no real review source), which risks a
  // Google manual action for fake reviews and would feed false data to AI crawlers. Real ratings
  // come from Judge.me, which emits its own review schema. The `ratings` flag is force-ignored
  // here regardless of persisted settings; re-enable only after wiring a REAL review source
  // (e.g. shop.metafields.judgeme.*).
  if (false && richAttributesSettings.ratings) {
    try {
...
async function installThemeSnippet(shop) {
  ...
  try {
    ...
    // IMPORTANT: schemas is stored as JSON string (array) in metafield with type "json"
    // In Shopify Liquid, JSON metafield .value returns the JSON string directly
    ...
    // OPTION 3: Storefront <head> JSON-LD injection is intentionally DISABLED.
  // The theme (Prestige) already emits a clean Product + Breadcrumb schema, and Judge.me
  // emits the real review/rating schema. Injecting a second Product/AggregateRating node
  // here caused duplicate/invalid structured data in Google Search. The app's JSON-LD is
  // still served to AI crawlers via the dedicated, Google-invisible AI endpoints
  // (/apps/indexaize/ai/schema-index.json and /ai/product/{handle}/schemas.json), which are
  // already advertised in llms.txt and allowed in robots.txt. This snippet now only emits
  // Google-safe <link rel="alternate"> discovery hints — NO <script type="application/ld+json">.
  const snippetContent = `{%- comment -%} AI Discovery Links - Auto-generated by indexAIze. Storefront JSON-LD injection is disabled by design; structured data is served to AI crawlers via the /apps/indexaize/ai/* endpoints. {%- endcomment -%}
{%- if shop.metafields.ai_discovery.settings -%}
  ...
  {%- if ai_settings.features.discoveryLinks -%}
    ...
    {%- endif -%}
    {%- comment -%} Per-product & index JSON-LD for AI crawlers (not rendered as on-page schema) {%- endcomment -%}
    {%- if product -%}
      <link rel="alternate" type="application/ld+json" href="/apps/indexaize/ai/product/{{ product.handle }}/schemas.json" title="AI Product Schema (JSON-LD)">
    {%- endif -%}
    <link rel="alternate" type="application/json" href="/apps/indexaize/ai/schema-index.json" title="AI Schema Index">
  {%- endif -%}
{%- endif -%}`;
...
router.get('/auto-inject.js', async (req, res) => {
  ...
  // OPTION 3: This legacy ScriptTag-based storefront injection is DISABLED.
  // It is no longer registered (installScriptTag is never called), but a ScriptTag may still
  // exist on stores from an older app version. Returning a no-op guarantees zero on-page JSON-LD
  // injection even in that case. Structured data is served to AI crawlers via /apps/indexaize/ai/*.
  res.setHeader('Content-Type', 'application/javascript');
  res.send('/* indexAIze: storefront JSON-LD injection disabled (Option 3). Schema is served via /apps/indexaize/ai/* endpoints. */');
});
