...
class AIDiscoveryService {
  ...
  async generateRobotsTxt(shop) {
    console.log('[ROBOTS] Generating for shop:', shop);
    
    // Зареди настройките от MongoDB
    const settings = await AIDiscoverySettings.findOne({ shop });
    console.log('[ROBOTS] MongoDB settings:', settings);
    
    if (!settings || !settings.enabled) {
      console.log('[ROBOTS] AI Discovery not enabled');
      return 'User-agent: *\nDisallow: /';
    }
    ...
    let robotsTxt = '# AI Discovery - Auto-generated\n\n';
    
    // Default rules
    robotsTxt += 'User-agent: *\nDisallow: /\n\n';
    
    // Rules for enabled bots
    enabledBots.forEach(bot => {
      robotsTxt += `# ${bot.toUpperCase()}\n`;
      robotsTxt += `User-agent: ${bot}Bot\n`;
      robotsTxt += 'Allow: /\n\n';
    });
